...
import { GitLabOAuthManager } from "./oauth.js";
...
// OAuth 2.0 PKCE Flow Configuration
const USE_OAUTH = process.env.USE_OAUTH === "true";
const GITLAB_OAUTH_SCOPES = process.env.GITLAB_OAUTH_SCOPES || "openid profile read_user write_repository api";
...
/**
 * Get authentication headers based on available authentication methods
 * Supports OAuth, token-based, and cookie authentication
 */
async function getAuthHeaders(sessionId?: string): Promise<Record<string, string>> {
  ...
  if (oauthManager && sessionId) {
    const accessToken = await oauthManager.getValidAccessToken(sessionId);
    if (accessToken) {
...
// Default headers for backward compatibility (when no session context)
...
/**
 * Make authenticated GitLab API request with session support
 */
async function makeAuthenticatedRequest(url: string, options: any = {}, sessionId?: string): Promise<any> {
  const fetchConfig = await createFetchConfig(sessionId);
...
// Legacy fetch config - will be replaced by OAuth-enabled version
const LEGACY_FETCH_CONFIG = {
...
// OAuth-enabled fetch config (dynamically created)
let DEFAULT_FETCH_CONFIG = LEGACY_FETCH_CONFIG;

// Define all available tools
const allTools = [

  {
    name: "oauth_login_pkce",
    description: "Initiate OAuth 2.0 authorization code flow with PKCE (exactly like glab CLI). Returns authorization URL that matches glab CLI behavior.",
    inputSchema: zodToJsonSchema(OAuthPKCESchema),
  },
  {
    name: "oauth_status",
    description: "Check OAuth authentication status and get user information for a session.",
    inputSchema: zodToJsonSchema(OAuthStatusSchema),
  },
  {
    name: "oauth_logout",
    description: "Logout and terminate an OAuth session.",
    inputSchema: zodToJsonSchema(OAuthLogoutSchema),
  },
  {
...
// Initialize OAuth PKCE flow if enabled
let oauthManager: GitLabOAuthManager | null = null;
async function initializeOAuth() {
  if (USE_OAUTH) {
    const gitlabBaseUrl = GITLAB_API_URL.replace('/api/v4', '');
    oauthManager = new GitLabOAuthManager(gitlabBaseUrl, GITLAB_OAUTH_SCOPES);

    // Wait a moment for the config to load
    await new Promise(resolve => setTimeout(resolve, 100));

    // Check if persistent session was loaded
    const persistentSessionId = oauthManager.getPersistentSessionId();
    if (persistentSessionId) {
      logger.info(`OAuth 2.0 PKCE authentication enabled with persistent session: ${persistentSessionId}`);
    } else {
      logger.info("OAuth 2.0 PKCE authentication enabled with token persistence");
    }
  }
}
...
initializeOAuth().catch(err => {
  logger.error('Failed to initialize OAuth:', err);
});

/**
 * Create fetch configuration with OAuth support and auto-retry (replaces DEFAULT_FETCH_CONFIG)
 */
async function createFetchConfig(sessionId?: string): Promise<any> {
...
/**
 * Enhanced fetch with OAuth token refresh and retry logic
 */
async function fetchWithOAuthRetry(url: string, options: any, sessionId?: string, maxRetries: number = 1): Promise<any> {
  let lastError: Error | null = null;

  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      // Get fresh config for each attempt
      const fetchConfig = await createFetchConfig(sessionId);
      const mergedOptions = { ...fetchConfig, ...options };

      const response = await fetch(url, mergedOptions);

      // If unauthorized and we have OAuth manager, try to refresh token
      if (response.status === 401 && sessionId && oauthManager && attempt < maxRetries) {
        logger.info('API call returned 401, attempting token refresh...');

        const refreshedToken = await oauthManager.getValidAccessToken(sessionId);
        if (refreshedToken) {
          ...
          throw new Error('Authentication expired. Please re-authenticate using oauth_login_pkce.');
        }
      }
      ...
      logger.warn(`API call attempt ${attempt + 1} failed, retrying...`, lastError.message);
    }
  }
...
/**
 * Update DEFAULT_FETCH_CONFIG to use OAuth when available
 */
async function updateDefaultFetchConfig(sessionId?: string): Promise<void> {
  if (sessionId && oauthManager) {
    try {
      const oauthConfig = await createFetchConfig(sessionId);
      DEFAULT_FETCH_CONFIG = oauthConfig;
      logger.debug('Updated DEFAULT_FETCH_CONFIG to use OAuth');
    } catch (error) {
      logger.warn('Failed to update DEFAULT_FETCH_CONFIG with OAuth, using legacy config');
      DEFAULT_FETCH_CONFIG = LEGACY_FETCH_CONFIG;
    }
  } else {
    DEFAULT_FETCH_CONFIG = LEGACY_FETCH_CONFIG;
  }
...
if (!GITLAB_PERSONAL_ACCESS_TOKEN && !oauthManager && !GITLAB_AUTH_COOKIE_PATH) {
  logger.error("No authentication method configured. Please set GITLAB_PERSONAL_ACCESS_TOKEN, configure OAuth, or provide GITLAB_AUTH_COOKIE_PATH");
  process.exit(1);
}
 ...
 */
async function getDefaultBranchRef(projectId: string, sessionId?: string): Promise<string> {
  projectId = decodeURIComponent(projectId); // Decode project ID
  const effectiveProjectId = getEffectiveProjectId(projectId);
  const url = new URL(`${GITLAB_API_URL}/projects/${encodeURIComponent(effectiveProjectId)}`);

  const response = await makeAuthenticatedRequest(url.toString(), {}, sessionId);
...
async function getFileContents(
  ...
  if (!ref) {
    ref = await getDefaultBranchRef(projectId, sessionId);
  }
...
async function getMergeRequest(
  ...
  const response = await fetchWithOAuthRetry(url.toString(), {}, sessionId);
...
server.setRequestHandler(CallToolRequestSchema, async request => {
  try {
    ...
    // Extract OAuth session ID from request arguments if available
    const sessionId = (request.params.arguments as any)?.sessionId as string | undefined;

    // Update DEFAULT_FETCH_CONFIG to use OAuth when sessionId is provided
    if (sessionId && oauthManager) {
      await updateDefaultFetchConfig(sessionId);
    }
    ...
    logger.info(request.params.name, sessionId ? `[Session: ${sessionId}]` : "");
    switch (request.params.name) {


      case "oauth_status": {
        if (!oauthManager) {
          throw new Error("OAuth is not configured. Please set USE_OAUTH=true.");
        }
        const args = OAuthStatusSchema.parse(request.params.arguments);

        try {
          const status = await oauthManager.getAuthStatus(args.sessionId);
          return {
            content: [
              {
                type: "text",
      ...
      case "oauth_logout": {
        ...
        const args = OAuthLogoutSchema.parse(request.params.arguments);

        try {
          await oauthManager.logout(args.sessionId);
          return {
            content: [
              {
                type: "text",
                text: JSON.stringify({
                  success: true,
                  message: "Successfully logged out and removed session."
                }, null, 2)
              }
            ]
          };
      ...
      case "oauth_login_pkce": {
        if (!oauthManager) {
          throw new Error("OAuth is not configured. Please set USE_OAUTH=true.");
        }
        const args = OAuthPKCESchema.parse(request.params.arguments);
        const sessionId = args.sessionId || randomUUID();

        try {
          const authFlow = await oauthManager.initiateOAuthFlow(sessionId);
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  ...
                  callbackUrl: authFlow.callbackUrl,
                  flow: "authorization_code_pkce",
                  message: "🔐 GitLab Authentication - Exact glab CLI Flow!",
                  quickStart: `Click here to authenticate: ${authFlow.authUrl}`,
                  instructions: [
                    "🚀 AUTHORIZATION CODE FLOW (Same as glab CLI):",
                    `   • Click this link: ${authFlow.authUrl}`,
                    "   • This is the EXACT same URL that glab CLI would open!",
                    "   • Uses PKCE for security (code_challenge_method=S256)",
                    "   • Redirects to localhost:7171/auth/redirect",
                    "",
                    "✅ NEXT STEPS:",
                    "   • Complete authentication in GitLab",
                    "   • You'll be redirected to localhost:7171 (callback)",
                    "   • Use oauth_status to check completion",
                    "",
                    "🔧 TECHNICAL DETAILS:",
                    "   • Flow: Authorization Code with PKCE",
                    "   • Scopes: openid profile read_user write_repository api",
                    "   • Client ID: Same as glab CLI",
                    "   • Redirect URI: http://localhost:7171/auth/redirect"
                  ],
                  technicalDetails: {
                    sessionId,
                    flow: "authorization_code_pkce",
                    clientId: "glab-cli-official",
                    scopes: "openid profile read_user write_repository api",
                    redirectUri: authFlow.callbackUrl,
                    callbackServerStarted: true,
                    callbackServerPort: 7171
                  }
                }, null, 2)
              }
            ]
          ...
          throw new Error(`Failed to initiate OAuth PKCE login: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
      ...
      case "search_repositories": {
        ...
        const results = await searchProjects(args.search, args.page, args.per_page, args.sessionId);
        return {
      ...
      case "get_file_contents": {
        ...
        const contents = await getFileContents(args.project_id, args.file_path, args.ref, args.sessionId);
        return {
      ...
      case "get_merge_request": {
        ...
        const sessionId = (request.params.arguments as any)?.sessionId;
...
// OAuth routes removed - OAuth is now handled via MCP tools (oauth_login, oauth_status, oauth_logout)
...
async function startSSEServer(): Promise<void> {
  ...
  // OAuth is now handled via MCP tools
